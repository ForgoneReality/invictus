...
export default class Ship {
  ...
  setup(type) {
    ...
    if (this.background.tutorial) {
      ...
      this.health =
        TYPES[type].health *
        LEVEL_MODIFIER[this.background.level_id][0] *
        DIFFICULTY_MODIFIER[this.background.difficulty][0];
      this.damage =
        TYPES[type].damage *
        LEVEL_MODIFIER[this.background.level_id][1] *
        DIFFICULTY_MODIFIER[this.background.difficulty][1];
    }
  ...
  move() {
    ...
    if (this.type === 0) {
      ...
      if (
        this.dependencies.length === 3 &&
        this.dependencies[2].duration <= 0
      ) {
      ...
      if (this.realX() > this.background.player.realX() + 30) {
        if (this.realX() <= this.background.width * 0.3) this.velX = 0;
        else this.velX = -1.5;
      } else if (this.realX() + 30 < this.background.player.realX()) {
        if (this.realX() >= this.background.width * 0.7) this.velX = 0;
        else this.velX = 1.5;
    ...
    switch (this.type) {
      case 0: //red ball center
        ...
        if (this.shootTimer <= 0) {
          ...
          audio.laser2.play();
          let proj = new CircleDamageProjectile(
            [this.realX(), this.realY() + 30],
            [speed * this.normalVector[0], speed * this.normalVector[1]],
            4.5,
            2,
            0,
            this.damage,
      ...
      case 1:
        ...
        if (this.shootTimer <= 0) {
          ...
          this.shootTimer = cooldown;
          let proj = new CircleDamageProjectile(
            [this.realX() + 40, this.realY() + 30],
            [(speed * Math.sqrt(2)) / 2, (speed * Math.sqrt(2)) / 2],
            6,
            2,
            1,
            this.damage,
          ...
          let proj2 = new CircleDamageProjectile(
            [this.realX() - 40, this.realY() + 30],
            [(speed * -1 * Math.sqrt(2)) / 2, (speed * Math.sqrt(2)) / 2],
            6,
            2,
            1,
            this.damage,
          ...
          let proj3 = new CircleDamageProjectile(
            [this.realX() + 40, this.realY() - 30],
            [(speed * Math.sqrt(2)) / 2, (-1 * speed * Math.sqrt(2)) / 2],
            6,
            2,
            1,
            this.damage,
          ...
          let proj4 = new CircleDamageProjectile(
            [this.realX() - 40, this.realY() - 30],
            [(speed * -1 * Math.sqrt(2)) / 2, (-1 * speed * Math.sqrt(2)) / 2],
            6,
            2,
            1,
            this.damage,
      ...
      case 2:
        ...
        if (this.shootTimer <= 0) {
          ...
          audio.laser2.play();
          let proj = new LaserDamageProjectile(
            [this.realX() + offset, this.realY() + 30],
            [0, speed],
            0,
            15,
            2,
      ...
      case 3:
        ...
        if (this.shootTimer <= 0) {
          ...
          let proj = new CircleDamageProjectile(
            [this.realX(), this.realY() + 30],
            [speed * scatter_x, speed * scatter_y],
            4.5,
            2,
            3,
            this.damage,
      ...
      case 4:
        ...
        if (this.shootTimer <= 0) {
          ...
          let proj = new CircleDamageProjectile(
            [this.realX() + rotate_scaler[0], this.realY() + rotate_scaler[1]],
            [speed * this.normalVector[0], speed * this.normalVector[1]],
            5,
            2,
            4,
          ...
          let proj2 = new CircleDamageProjectile(
            [
              this.realX() + rotate_scaler2[0],
              this.realY() + rotate_scaler2[1],
            ],
            [speed * this.normalVector[0], speed * this.normalVector[1]],
            5,
            2,
      ...
      case 6: //red ball center
        ...
        if (this.shootTimer <= 0) {
          ...
          let proj2 = new LaserDamageProjectile(
            [this.realX() - 122, this.realY() + 110],
            [0, speed],
            0,
            41,
        ...
        if (this.shootTimer2 <= 0) {
          ...
          let proj3 = new LaserBeam(
            [this.realX(), this.realY() + 65],
            [0, 0],
            this.canvasBottom,
            70,
            7.5 *
        ...
        if (this.shootTimer2 === 100) {
          ...
          let flare = new LensFlare(
            [this.realX(), this.realY() + 93.5],
            98.5,
            "images/redflare.png",
            0.12,
            "#8b0000",
      ...
      case 7:
        ...
        if (this.shootTimer <= 0) {
          ...
          let proj3 = new LaserDamageProjectile(
            [this.realX() + rotate_scaler[0], this.realY() + rotate_scaler[1]],
            [speed * this.normalVector[0], speed * this.normalVector[1]],
            this.degrees + 180,
            30,
          ...
          let proj4 = new LaserDamageProjectile(
            [
              this.realX() + rotate_scaler2[0],
              this.realY() + rotate_scaler2[1],
            ],
      ...
      case 9:
        ...
        if (this.shootTimer <= 0) {
          ...
          let proj = new EllipseDamageProjectile(
            [this.realX() - 55, this.realY() + 80],
            [0, speed],
            3,
            50,
            0,
          ...
          let proj2 = new EllipseDamageProjectile(
            [this.realX() + 55, this.realY() + 80],
            [0, speed],
            3,
            50,
      ...
      case 10:
        ...
        if (this.shootTimer <= 0) {
          ...
          let proj = new LaserDamageProjectile(
            ...
            15,
            3,
            "images/attackprojectile1.png"
          );
          let proj2 = new LaserDamageProjectile(
            [this.realX() - 42, this.realY() + 30],
            [(-1 * speed * Math.sqrt(2)) / 2, (speed * Math.sqrt(2)) / 2],
            45,
            -1,
            2,
            4,
      ...
      case 11:
        ...
        if (this.shootTimer3 <= 0) {
          ...
          let proj = new OrbDamageProjectile(
            [this.realX() + rotate_scaler[0], this.realY() + rotate_scaler[1]],
            [
              Math.cos((scatter_deg * Math.PI) / 180) * speed,
              Math.sin((scatter_deg * Math.PI) / 180) * speed,
            ],
            18,
            0,
            damage,
          ...
          let proj2 = new OrbDamageProjectile(
            [
              this.realX() + rotate_scaler2[0],
              this.realY() + rotate_scaler2[1],
            ],
        ...
        if (this.shootTimer <= 0) {
          ...
          let proj = new LaserDamageProjectile(
            [this.realX() + rotate_scaler[0], this.realY() + rotate_scaler[1]],
            [this.normalVector[0] * speed, this.normalVector[1] * speed],
            this.degrees + 180,
            30,
            2,
            5,
        ...
        if (this.shootTimer2 <= 0) {
          let offset_x = -165;
          let offset_y = 215;
          let my_degrees1 = this.customAngleAndNormalizedVector(
            offset_x,
            offset_y
          )[0];
          let my_degrees2 = this.customAngleAndNormalizedVector(
            -1 * offset_x,
            offset_y
          )[0];
          ...
          let proj = new CircleDamageProjectile(
            [this.realX() + rotate_scaler[0], this.realY() + rotate_scaler[1]],
            [speed * scatter_x1, speed * scatter_y1],
            4,
            2,
            7,
            damage,
          ...
          let proj2 = new CircleDamageProjectile(
            ...
            [speed * scatter_x2, speed * scatter_y2],
            4,
            2,
            7,
            damage,
  ...
  realX() {
    //INCOMPLETE - NEEDS TO FACTOR IN ANGLE!! ... or does it? center doesn't change when angle changes
    return this.posX + this.width / 2;
  }

  realY() {
    if (this.type === 1) {
      return this.posY + this.height * 0.4;
    } else return this.posY + this.height / 2;
  }

  //#offset returns the new (x,y) relative to center after rotating to this.degrees
  offset(
    x,
    y //x,y relative to center, which should be realX() and real(Y), prior to rotation
  ) {
    //new stuff below
    if (
      (this.degrees === undefined ||
        this.degrees === null ||
        Number.isNaN(this.degrees)) &&
      this.rotatable
    ) {
      //shouldn't need this but jic
  ...
  downY() {
    if (this.type === 6 || this.type === 11) {
      return this.posY + this.height * 0.8;
    }
    return this.posY + this.height;
  }

  corners() {
    let topleft_corner = this.offset(
      this.leftX() - this.realX(),
      this.upY() - this.realY()
    );

    let topright_corner = this.offset(
      this.rightX() - this.realX(),
      this.upY() - this.realY()
    );
    let bottomleft_corner = this.offset(
      this.leftX() - this.realX(),
      this.downY() - this.realY()
    );
    let bottomright_corner = this.offset(
      this.rightX() - this.realX(),
      this.downY() - this.realY()
    );

    return [
      [this.realX() + topleft_corner[0], this.realY() + topleft_corner[1]],
      [this.realX() + topright_corner[0], this.realY() + topright_corner[1]],
      [
        this.realX() + bottomleft_corner[0],
        this.realY() + bottomleft_corner[1],
      ],
      [
        this.realX() + bottomright_corner[0],
        this.realY() + bottomright_corner[1],
      ],
    ];
  }
}
